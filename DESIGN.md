# DESIGN.md: Awesome STPA

## Product

**Name:** Awesome STPA  
**Purpose:** Public-domain curated index of STAMP/STPA, CAST, and hazard-analysis resources: handbooks, tools, standards context, case studies, learning, and datasets.  
**Primary users:** Safety and systems engineering practitioners.  
...
**Surface:** One GitHub Pages landing (`docs/index.html` path family) with in-page anchors. README on GitHub stays the canonical deep list; the landing does not mirror entry rows.
...
Derivation: this contract reuses the awesome-archimate DESIGN system unchanged (family constraint: one design system across the awesome-mbse spokes). Only the product facts above differ.
...
Derivation stays inside minimal-technical ranges (cool neutrals, accent hue 250-256). Concrete tokens:
...
| Need | Source |
|---|---|
| Page shell, nav, footer | Custom, tokens above |
| Primary/secondary buttons | Custom button styles mapped to primary/accent; if shadcn Button appears later, retheme every token before ship |
| Section index links | Custom anchor list; Primer density as reference only |
| Entry rows | Not on the landing; README owns entry rows (title link + mono tag chips + year) |
...
- Default is typography + list; no diagram earns space on this landing  
- Open Graph: title + description; no social image for the pilot

## Responsive Behaviour

- **Desktop (>=1024px):** full section index density; two-column index  
...
- Touch targets >=44px on nav links and primary actions  
- `lang="en"`; semantic landmarks (`header`, `main`, `nav`, `footer`); heading order h1 -> h2  
- Reduced motion respected as above

## Component Sources

1. Existing project landing HTML/CSS (`docs/index.html`, restyled to this contract)  
2. awesome-archimate `docs/index.html` (family reference implementation)  
3. shadcn/ui only on a future app path, rethemed  
4. No marketing kits on this list product
...
**Pilot composition order:** nav -> hero (claim + one primary CTA) -> evidence strip (v0.1.0, last sweep 2026-09, entry count 42, text-only family pointer) -> section index (primary) -> short curation note -> contribute links -> footer (CC0, licence enquiry, maintainer).

**Implementation note (not style):** Path S static `docs/` with self-hosted Plex (OFL licence disclosed in `docs/fonts/OFL.txt`) is the build; Path N Next export stays out until a spoke needs app components.

```text
Files read:
- awesome-archimate/DESIGN_BRIEF.md - family brief pattern (audience, style, avoid)
- awesome-archimate/DESIGN.md - full token, layout, motion, a11y contract reused unchanged
- awesome-archimate/docs/index.html - reference landing implementation
- README.md - section headings, entry grammar, sweep badge, support channels
- RELEASE-INFO.txt - version truth for evidence chips
```
